...
async def check_timers(id_user):
    ...
    await timer_cur.execute(
        f"SELECT message, id, datetime FROM timers WHERE user_id = {id_user} and datetime > CURRENT_TIMESTAMP"
    )
...
async def get_all_timers_by_user(id_user):
    ...
    timer_cur = await db.cursor()
    await timer_cur.execute(
        f"SELECT message, id, datetime FROM timers WHERE user_id = {id_user}"
    )
    user_timers = await timer_cur.fetchall()
    await db.commit()
    await db.close()
